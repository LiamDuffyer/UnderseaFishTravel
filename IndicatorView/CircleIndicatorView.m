...
- (void)updateUI {
    NSString *valueString = [NSString stringWithFormat:@"%@%@", @(self.centerValue), @"dB"];
    NSMutableAttributedString *stringM = [[NSMutableAttributedString alloc] initWithString:valueString];
    NSRange range = [valueString rangeOfString:@"dB"];
    [stringM addAttribute:NSForegroundColorAttributeName value:UIColor.darkGrayColor range:NSMakeRange(0, stringM.length)];
    ...
    [stringM addAttribute:NSFontAttributeName value:DefaultFont(18.0 / 56.0 * self.centerValueLabelFontSize) range:NSMakeRange(range.location, 2)]; 
...
- (void)setDataThatIsRelatedToFrameToCustomRatio {
    ...
    self.centerValueLabelXOffset = 5.0 * ApplyRatio(self.centerValueLabelXOffset_ratio);
    self.centerValueLabelYOffset = 11.0 * ApplyRatio(self.centerValueLabelYOffset_ratio);
    self.hotStatusButtonWidth = 30.0 * ApplyRatio(self.hotStatusButtonWidth_ratio);
    self.hotStatusButtonHeight = 40.0 * ApplyRatio(self.hotStatusButtonHeight_ratio);
...
- (CAShapeLayer *)maskLayerForLayer3 {
    ...
    for (int i = 0; i < self.innerAnnulusValueToShowArray.count; i++) {
        ...
        scaleLabel.text = [NSString stringWithFormat:@"%@ dB", self.innerAnnulusValueToShowArray[i]];
